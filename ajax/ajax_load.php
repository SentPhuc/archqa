...
?>
<?php if($type == 'video-click') {
	$width =(isset($_GET["width"])) ? htmlspecialchars($_GET["width"]) : '';
	$height = (isset($_GET["height"])) ? htmlspecialchars($_GET["height"]) : '';
	...
	<div class="sub-video1">
		<iframe title="<?=$videonb[0]['ten'.$lang]?>" src="//www.youtube.com/embed/<?=$func->getYoutube($videonb[0]['link_video'])?>" width="<?=$width?>" height="<?=$height?>" frameborder="0" allowfullscreen></iframe>
	</div>
	<div class="sub-video2">
		<div class="slick-video">
			<?php for($i=0; $i<count($videonb); $i++) { ?>
				<a class="item-video" data-width="<?=$width?>" data-height="<?=$height?>" data-id="<?=$videonb[$i]['id']?>">
					<img src="//img.youtube.com/vi/<?=$func->getYoutube($videonb[$i]['link_video'])?>/0.jpg"/>
				</a>
			<?php } ?>
		</div>
	</div>
	<script type="text/javascript">
	    $(document).ready(function(){
	    	$('.slick-video').slick({slidesToShow: 3,arrows:false});
	    });
	</script>
<?php } } ?>
        ...
        $(document).ready(function()
        {
            $('.listvideos').change(function() 
            {
                ...
                $.ajax({
		            ...
		            type: "POST",
		            dataType: 'html',
		            data: {id:id,height:"100%",width:"100%"},
