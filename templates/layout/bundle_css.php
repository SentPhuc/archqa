...
<?php
    ...
    if($bgbody['hienthi'])
    {
        ...
        {
            echo '<style type="text/css">.wrap-newsletter{background: url('.UPLOAD_PHOTO_L.$bgbody['photo'].') '.$bgbodyOptions['repeat'].' '.$bgbodyOptions['position'].' '.(($bgbodyOptions['attachment']!=0)?$bgbodyOptions['attachment']:"").' ;background-size:'.$bgbodyOptions['size'].'}</style>';
        }
        else
        {
