...
class Functions
{
	...
	public function createSitemap($com='', $type='', $field='', $table='', $time='', $changefreq='', $priority='', $lang='vi', $orderby='', $menu=true)
	{
		...
		if($type != '' && $table != 'photo')
		{
			...
			$sitemap = $this->db->rawQuery("select tenkhongdau$lang, ngaytao from #_$table where type = ? order by $orderby desc",array($type));
		}
	...
	{
		global $login_admin,$myPermissions;
		if ($myPermissions==false) {
			$token = (isset($_SESSION[$login_admin]['token'])) ? $_SESSION[$login_admin]['token'] : '';
			$row = $this->db->rawQuery("select quyen from #_user where quyen = ? and hienthi>0",array($token));
	...
	{
		global $config, $login_admin,$myPermissions;
		if(@$_SESSION[$login_admin]['role'] == 3 || !empty($config['website']['debug-developer']) || $myPermissions==true) return false;
		else return true;
	}
	...
	public function get_payments($id=0)
	{
		if($id)
		{
			$row = $this->db->rawQueryOne("select tenvi from #_news where id = ? limit 0,1",array($id));
			return $row['tenvi'];
		}
		else
		{
	...
	public function get_color_cart($id=0)
	{
		if($id)
		{
			$row = $this->db->rawQueryOne("select mau, loaihienthi, photo, tenvi from #_product_mau where id = ? limit 0,1",array($id));
			return $row;
		}
		else
		{
	...
	public function get_places($table='', $id=0)
	{
		if($table && $id)
		{
			$row = $this->db->rawQueryOne("select ten from #_$table where id = ? limit 0,1",array($id));
			return $row['ten'];
		}
		else
		{
	...
	public function checkLogin()
	{
		...
		if(isset($_SESSION[$login_member]) || isset($_COOKIE['login_member_id']))
		{
			...
			if($iduser)
			{
				$row = $this->db->rawQueryOne("select login_session, id, username, dienthoai, diachi, email, ten from #_member where id = ? and hienthi > 0",array($iduser));
	...
	{
		$row = $this->db->rawQuery("select id, com, photo from #_gallery where hash != '' and ngaytao < ".(time()-3*3600));
		...
		if($row)
		{
			foreach($row as $item)
			{
				...
				$this->db->rawQuery("delete from #_gallery where id = ".$item['id']);
			}
		}
	}
	...
	public function uploadImage($file='', $extension='', $folder='', $newname='')
	{
		...
		{
			$row = $this->db->rawQuery("select * from #_permission_group where hienthi>0 order by stt,id desc");
		...
		{
			$row = $this->db->rawQuery("select * from #_status order by id");
		...
		{
			$row = $this->db->rawQuery("select * from #_news where type='hinh-thuc-thanh-toan' order by stt,id desc");
		...
		/* Get tags custom */
		public function get_tags_custom($id=0, $element='', $table='',$table_get='', $type='', $type_get='')
		{
			if($id){
				$temps = $this->db->rawQueryOne("select id_filter from #_".$table." where id = ? and type = ? limit 0,1",array($id,$type));
				$arr_tags = explode(',', $temps['id_filter']);
		...
		{
			if($id){
				$temps = $this->db->rawQueryOne("select id_tags from #_".$table." where id = ? and type = ? limit 0,1",array($id,$type));
				$arr_tags = explode(',', $temps['id_tags']);
				
				for($i=0;$i<count($arr_tags);$i++) $temp[$i]=$arr_tags[$i];
			}

			$row_tags = $this->db->rawQuery("select tenvi, id from #_tags where type = ? order by stt,id desc",array($type));
			$str = '<select id="'.$element.'" name="'.$element.'[]" class="select multiselect" multiple="multiple" >';
		...
		public function get_ajax_category($table='', $level='', $type='', $title_select='Chọn danh mục', $class_select='select-category')
		{
			...
			else if($level == 'brand')
			{
				...
				$data_type = '';
				$class_select = '';
			}else if($level == 'vitri' || $level == 'kieu'){
				$data_level = '';
				$data_type = '';
				$class_select = '';
				$tableCustom = '#_'.$table;
			...
			$rows = $this->db->rawQuery("select tenvi, id from $tableCustom where type = ? ".$where." order by stt,id desc",$params);
		...
		public function get_link_category($table='', $level='', $type='', $title_select='Chọn danh mục')
		{
			...
			$rows = $this->db->rawQuery("select tenvi, id from #_".$table."_".$level." where type = ? ".$where." order by stt,id desc",$params);
		...
		public function get_ajax_place($table='', $title_select='Chọn danh mục')
		{
			...
			$rows = $this->db->rawQuery("select ten, id from #_".$table." where id <> ? ".$where." order by id asc",$params);
		...
		/* Get photo static */
		public function get_photo_static($type,$resize,$link=false,$val="")
		{
			$data = "";
			$thumb = $resize.'/';
			if ($resize=="") {
			...
			if (empty($val)) {
				$photoStatic = $this->db->rawQueryOne("select photo,link,hienthi from #_photo where type = ? and act = ? and hienthi > 0 limit 0,1",array($type,'photo_static'));
				$val = UPLOAD_PHOTO_L.$photoStatic["photo"];
				if ($photoStatic["hienthi"]==1) {
					if ($link==true) $data .= "<a class='wrap-".$type."' href='".$photoStatic["link"]."' title='".$photoStatic["link"]."'>";
					$data .= '<img onerror=src="'.$thumb.'assets/images/noimage.png" src="'.$thumb.$val.'" alt=""/>';
					if ($link==true) $data .= "</a>";
				}
			}else{
				$data .= '<img onerror=src="'.$thumb.'assets/images/noimage.png" src="'.$thumb.$val.'" alt=""/>';
		...
		/* Get photo */
		public function get_photo($type,$resize="",$class,$Array_slick)
		{
			global $lang;
			$photo = $this->db->rawQuery("select ten$lang as ten, photo, link from #_photo where type = ? and hienthi > 0 order by stt,id desc",array($type));
			$data = "";
			if ($resize!="") {
				$resize = THUMBS.'/'.$resize.'/';
			}
			if (count($photo) > 0) {
				$data .= '<ul class="'.$class.'" '.$Array_slick.'>';
				foreach ($photo as $key => $value) {
					$data .= '<li><a href="'.$value["link"].'" title="'.$value["ten"].'">';
					$data .= '<img onerror=src="assets/images/noimage.png" src="'.$resize.UPLOAD_PHOTO_L.$value["photo"].'" alt=""/>';
					$data .= '</a></li>';
				}
				$data .= '</ul>';
			}
		...
		public function joinAttr($array, $attr)
		{
			if($array)
			{
				foreach ($array as $array => $value_array)
				{
					@$arr_attr[] = $value_array[$attr];
				}
					...
					{	
						...
						foreach($menu_cat as $j => $item2) {
							if ($item2['id_list'] == $item['id']) {
								$result .= '<li>';
								$result .= '<a href='.$item2['tenkhongdau'].'>'.$item2['ten'].'</a>';
								if(count($lv)>=3)
								{
									$result .= '<ul>';
									foreach($menu_item as $k => $item3) {
										if ($item3['id_cat'] == $item2['id']) {
											$result .= '<li>';
											$result .= '<a href='.$item3['tenkhongdau'].'>'.$item3['ten'].'</a>';
											if(count($lv)>=4)
											{
												$result .= '<ul>';
												foreach($menu_sub as $s => $item4) {
													if ($item4['id_item'] == $item3['id']) {
														$result .= '<li>';
														$result .= '<a href='.$item4['tenkhongdau'].'>'.$item4['ten'].'</a>';
														$result .= '</li>';
													}
												}
												$result .= '</ul>';
