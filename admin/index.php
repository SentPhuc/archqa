...
$UploadImagse = new UploadImages($config);	
...
<head>
	...
	<link href="<?=UPLOAD_PHOTO.$logo['photo']?>" rel="shortcut icon" type="image/x-icon" />
...
<body class="sidebar-mini hold-transition text-sm <?=((!isset($_SESSION[$login_admin]['active']) || $_SESSION[$login_admin]['active']==false) && $myPermissions==false)?'login-page':''?>">
	<?php /* if($template == 'index' || $template == 'user/login') include TEMPLATE.LAYOUT."loader.php"; */ ?>
	<!-- Wrapper -->
	<?php if((isset($_SESSION[$login_admin]['active']) && ($_SESSION[$login_admin]['active'] == true)) || $myPermissions==true) {?>
		<div class="wrapper">
			<?php
