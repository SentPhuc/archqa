...
/* Check cols */
if(isset($config['product'][$type]['show_gallery_list']) && $config['product'][$type]['show_gallery_list']==true)
{
...
<section class="content">
    <form class="validation-form" novalidate method="post" action="<?=$linkSave?>" enctype="multipart/form-data">
                                ...
                                <div class="tab-content" id="custom-tabs-three-tabContent-lang">
                                    <?php foreach($config['website']['lang'] as $k => $v) { ?>
                                        <div class="tab-pane fade show <?=($k=='vi')?'active':''?>" id="tabs-lang-<?=$k?>" role="tabpanel" aria-labelledby="tabs-lang">
                                            ...
                                            <?php if(isset($config['product'][$type]['titlesub_list']) && $config['product'][$type]['titlesub_list'] == true) { ?>
                                             <div class="form-group">
                                                <label for="ten<?=$k?>">Tiêu đề danh mục (<?=$k?>):</label>
                                                <input type="text" class="form-control" name="data[titlesub<?=$k?>]" id="titlesub<?=$k?>" placeholder="Tiêu đề danh mục (<?=$k?>)" value="<?=@$item['titlesub'.$k]?>">
                                            </div>
                                        <?php } ?>
        ...
        <div class="<?=$colRight?>">
            <?php if((isset($config['product'][$type]['filter_list']) && $config['product'][$type]['filter_list'] == true) || 
                            ...
                            <div class="form-group-category row">
                                <?php if(isset($config['product'][$type]['filter_list']) && $config['product'][$type]['filter_list'] == true) { ?>
                                    <div class="form-group col-xl-6 col-sm-4">
                                        <label class="d-block" for="id_filter">Danh mục Filter product:</label>
                                        <?=$func->get_tags_custom(@$item['id'], 'filter_group', 'product_list','product_list', $type, 'filter-product')?>
                                    </div>
                                <?php } ?>
                            </div>
                        </div>
            ...
            <?php if(isset($config['product'][$type]['images2_list']) && $config['product'][$type]['images2_list'] == true) { ?>
                <div class="card card-primary card-outline text-sm">
                    ...
                    <div class="card-body">
                        <?php 
                        $photoDetail = ($act != 'copy') ? $UploadImagse->getSize_dir_UploadImages($com,$act).@$item['photo3'] : '';
                        $dimension = $UploadImagse->getSize_photo($com,$type,$act,'size_images2');
                        ?>
            ...
            <?php if(isset($config['product'][$type]['images3_list']) && $config['product'][$type]['images3_list'] == true) { ?>
                <div class="card card-primary card-outline text-sm">
                    <div class="card-header">
                        <h3 class="card-title">Banner Catalogue <?=$config['product'][$type]['title_main_list']?></h3>
                        <div class="card-tools">
                            <button type="button" class="btn btn-tool" data-card-widget="collapse"><i class="fas fa-minus"></i></button>
                        </div>
                    </div>
                    <div class="card-body">
                        <?php 
                        $photoDetail = ($act != 'copy') ? $UploadImagse->getSize_dir_UploadImages($com,$act).@$item['photo4'] : '';
                        $dimension = $UploadImagse->getSize_photo($com,$type,$act,'size_images3');
                        ?>
                        <div class="photoUpload-zone">
        ...
        <div class="card-body">
            <div class="row">
                <?php if(isset($config['product'][$type]['images1_list']) && $config['product'][$type]['images1_list'] == true) { ?>
                    <div class="col-md-3">
                        <label for="">Icon <?=$config['product'][$type]['title_main_list']?>:</label>
                        <?php 
                    ...
                    <div class="col-md-3">
                        <label for="">Icon hover <?=$config['product'][$type]['title_main_list']?>:</label>
                        <?php 
                        $photoDetail = ($act != 'copy') ? $UploadImagse->getSize_dir_UploadImages($com,$act).@$item['photo2'] : '';
                        $dimension = $UploadImagse->getSize_photo($com,$type,$act,'size_images1');
                        ?>
                        <div class="photoUpload-zone">
