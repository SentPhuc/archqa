            ...
            <table class="table table-hover">
                <thead>
                    <tr>
                        ...
                        <?php } ?>
                        <?php if(isset($config['product'][$type]['like']) && count($config['product'][$type]['like']) > 0) { ?>
                            <th class="align-middle text-center">Số lượt like</th>
                        <?php } ?>
                        <?php if(isset($config['product'][$type]['save']) && count($config['product'][$type]['save']) > 0) { ?>
                            <th class="align-middle text-center">Số lượt yêu thích</th>
                        <?php } ?>
                            ...
                            <tr>
                                ...
                                <?php if(isset($config['product'][$type]['like']) && count($config['product'][$type]['like']) > 0) { ?>
                                    <td class="align-middle text-center">
                                        <h6 class="mb-0"><?=($items[$i]['countLike']) ? $items[$i]['countLike'] : 0?></h6>
                                    </td>
                                <?php } ?>
                                <?php if(isset($config['product'][$type]['save']) && count($config['product'][$type]['save']) > 0) {
                                    $getProductSave = $d->rawQuery("select id from #_product_save where value > 0 and type = ? and id_pro = ?",array($type,$items[$i]['id']));
                                    ?>
                                    <td class="align-middle text-center">
                                        <h6 class="mb-0"><?=($getProductSave) ? count($getProductSave) : 0?></h6>
                                    </td>
                                <?php }?>
                                <?php if(isset($config['product'][$type]['check'])) { foreach($config['product'][$type]['check'] as $key => $value) { ?>
                                    <td class="align-middle text-center">
