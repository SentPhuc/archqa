		...
		<?php for($i=0;$i<$numberPhoto;$i++) { $stt = $i+1; ?>
			<div class="card card-primary card-outline text-sm">
	            ...
	            <div class="card-body">
	                ...
	                <?php } ?>
	                <?php if(isset($config[$com][$type][$dfgallery][$val]['value_photo']) && $config[$com][$type][$dfgallery][$val]['value_photo'] == true) { ?>
						<div class="form-group">
		                    <label for="value<?=$i?>">Giá trị thống kê:</label>
		                    <input type="text" class="form-control" name="dataMulti[<?=$i?>][value]" id="value<?=$i?>" placeholder="Giá trị thống kê">
		                </div>
	                <?php } ?>
		                                ...
		                                <div class="tab-pane fade show <?=($k=='vi')?'active':''?>" id="tabs-lang-<?=$k?>-<?=$i?>" role="tabpanel" aria-labelledby="tabs-lang">
		                                    ...
		                                    <?php if(isset($config[$com][$type][$dfgallery][$val]['mota_photo']) && $config[$com][$type][$dfgallery][$val]['mota_photo'] == true) { ?>
		                                        <div class="form-group">
		                                            ...
		                                            <textarea class="form-control" name="dataMulti[<?=$i?>][mota<?=$k?>]" id="mota<?=$k?><?=$i?>" rows="5" placeholder="Mô tả (<?=$k?>)"></textarea>
		                                        </div>
		                                    <?php } ?>
		                                </div>
		                            <?php } ?>
