...
<aside class="main-sidebar sidebar-dark-primary elevation-4 text-sm">
    ...
    <a class="brand-link" href="index.php">
        <img onerror="this.src='<?=THUMBS?>/120x100x2/assets/images/noimage.png';" src="<?=UPLOAD_PHOTO.$logo['photo']?>"/>
    </a>
    ...
    <div class="sidebar">
        <nav class="mt-3">
            <ul class="nav nav-pills nav-sidebar flex-column nav-child-indent text-sm" data-widget="treeview" role="menu" data-accordion="false">
                ...
                <?php $disabled = array(); if(isset($config['group'])) { foreach($config['group'] as $key => $value) { ?>
                    <li class="nav-item has-treeview menu-group">
                        ...
                        <ul class="nav nav-treeview">
                            ...
                            <?php } } ?>
                            <?php if(isset($value['project'])) { foreach($value['project'] as $k) { ?>
                                <?php
                                    $disabled['project'][$k] = 1;
                                    $v = $config['project'][$k];
                                    $none = "";
                                    $active = "";
                                    $menuopen = "";
                                    if(isset($kiemtra) && $kiemtra == true) if($func->check_access('project', 'man_list', $k, null, 'phrase-1') && $func->check_access('project', 'man', $k, null, 'phrase-1')) $none = "d-none";
                                    if(($com=='project') && ($k==$_GET['type']))
                                    {
                                        $active = 'active';
                                        $menuopen = 'menu-open';
                                    }
                                ...
                                <li class="nav-item has-treeview <?=$menuopen?> <?=$none?>">
                                    ...
                                    <ul class="nav nav-treeview">
                                        <?php if(!empty($v['dropdown'])) { if(isset($v['list']) && $v['list'] == true) {
                                            $none = "";
                                            $active = "";
                                            if(isset($kiemtra) && $kiemtra == true) if($func->check_access('project', 'man_list', $k, null, 'phrase-1')) $none = "d-none";
                                            if($com=='project' && ($act=='man_list' || $act=='add_list' || $act=='edit_list' || $kind=='man_list') && $k==$_GET['type']) $active = "active"; ?>
                                            <li class="nav-item <?=$none?>"><a class="nav-link <?=$active?>" href="index.php?com=project&act=man_list&type=<?=$k?>" title="Danh mục cấp 1"><i class="nav-icon text-sm far fa-caret-square-right"></i><p>Danh mục cấp 1</p></a></li>
                                        <?php } } ?>
                                        <?php
                                            $none = "";
                                            $active = "";
                                            if(isset($kiemtra) && $kiemtra == true) if($func->check_access('project', 'man', $k, null, 'phrase-1')) $none = "d-none";
                                            if($com=='project' && ($act=='man' || $act=='add' || $act=='edit' || $act=='copy' || $kind=='man') && $k==$_GET['type']) $active = "active";
                                        ?>
                                        <li class="nav-item <?=$none?>"><a class="nav-link <?=$active?>" href="index.php?com=project&act=man&type=<?=$k?>" title="<?=$v['title_main']?>"><i class="nav-icon text-sm far fa-caret-square-right"></i><p><?=$v['title_main']?></p></a></li>
                                    </ul>
                                </li>
                            <?php } } ?>
                ...
                <!-- Dự án -->
                <?php if(isset($config['project'])) { ?>
                    <?php foreach($config['project'] as $k => $v) { if(!isset($disabled['project'][$k])) { ?>
                        <?php
                            $none = "";
                            $active = "";
                            $menuopen = "";
                            if(isset($kiemtra) && $kiemtra == true) if($func->check_access('project', 'man_list', $k, null, 'phrase-1') && $func->check_access('project', 'man_cat', $k, null, 'phrase-1') && $func->check_access('project', 'man_item', $k, null, 'phrase-1') && $func->check_access('project', 'man_sub', $k, null, 'phrase-1') && $func->check_access('project', 'man_brand', $k, null, 'phrase-1') && $func->check_access('project', 'man', $k, null, 'phrase-1') && $func->check_access('import', 'man', $k, null, 'phrase-1') && $func->check_access('export', 'man', $k, null, 'phrase-1')) $none = "d-none";
                            if((($com=='project') || ($com=='import') || ($com=='export')) && ($k==$_GET['type']))
                            {
                                $active = 'active';
                                $menuopen = 'menu-open';
                            }
