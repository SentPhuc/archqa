...
<section class="content">
    <form class="validation-form" novalidate method="post" action="<?=$linkSave?>" enctype="multipart/form-data">
                                ...
                                <div class="tab-content" id="custom-tabs-three-tabContent-lang">
                                    <?php foreach($config['website']['lang'] as $k => $v) { ?>
                                        <div class="tab-pane fade show <?=($k=='vi')?'active':''?>" id="tabs-lang-<?=$k?>" role="tabpanel" aria-labelledby="tabs-lang">
                                            ...
                                            <?php if(isset($config['news'][$type]['tieude_category']) && $config['news'][$type]['tieude_category'] == true) { ?>
                                                <div class="form-group">
                                                    <label for="tieude_category<?=$k?>">Tiêu đề Category (<?=$k?>):</label>
                                                    <input type="text" class="form-control" name="data[tieude_category<?=$k?>]" id="tieude_category<?=$k?>" placeholder="Tiêu đề Category (<?=$k?>)" value="<?=@$item['tieude_category'.$k]?>">
                                                </div>
                                            <?php } ?>
            ...
            <div class="<?=$colRight?>">
                        ...
                        <div class="card-body">
                            ...
                            <?php if (!empty($item['link_video'])) {?>
                                <div class="form-group">
                                    ...
                                    <div><iframe id="loadVideo" width="100%" src="//www.youtube.com/embed/<?=$func->getYoutube($item['link_video'])?>" <?=(@$item["link_video"] == '')?"height='0'":"height='250'";?> frameborder="0" allowfullscreen></iframe></div>
                                </div>
                            <?php } ?>
                        </div>
                    </div>
                ...
                <?php if(isset($config['news'][$type]['images1']) && $config['news'][$type]['images1'] == true) { ?>
                    <div class="card card-primary card-outline text-sm">
                        ...
                        <div class="card-body">
                            <?php 
                            $photoDetail = ($act != 'copy') ? $UploadImagse->getSize_dir_UploadImages($com,$act).@$item['photo1'] : '';
                            $dimension = $UploadImagse->getSize_photo($com,$type,$act,'size_images1');
                            ?>
                            <div class="photoUpload-zone">
                                <div class="photoUpload-detail" id="photoUpload-preview1">
                                    <img class="rounded" src="<?=@$photoDetail?>" onerror="src='assets/images/noimage.png'" alt="Alt Photo"/>
                                </div>
        ...
        <div class="card card-primary card-outline text-sm">
            ...
            <div class="card-body">
                <div class="row">
                    <?php if(isset($config['news'][$type]['theme']) && $config['news'][$type]['theme'] == true) { ?>
                        <div class="form-group col-md-3">
                            <label for="theme" class="d-inline-block align-middle mb-2">Chọn loại hiển thị:</label>
                            <select class="form-control" id="theme" name="data[theme]">
                                <option value="0">Mặc định</option>
                                <option value="1">Loại 2</option>
                                <option value="2">Loại slide</option>
                            </select>
                        </div>
                    <?php } ?>
                    <?php if(isset($config['news'][$type]['dientich']) && $config['news'][$type]['dientich'] == true) { ?>
                        <div class="form-group col-md-3">
                            <label class="d-block" for="dientich">Diện tích <?=($type=='loaicongtrinh') ? "(S < ".@$item['dientich']."m2)":""?>:</label>
                            <div class="input-group">
                                <input type="text" class="form-control" name="data[dientich]" id="dientich" placeholder="Diện tích <?=($type=='loaicongtrinh') ? "(S < ".@$item['dientich']."m2)":""?>" value="<?=@$item['dientich']?>">
                                <div class="input-group-append">
                                    <div class="input-group-text"><strong><?=@$item['dientich']?>m2</strong></div>
                                </div>
                            </div>
                        </div>
                    <?php } ?>
                    <?php if(isset($config['news'][$type]['phantram_dongia']) && $config['news'][$type]['phantram_dongia'] == true) { ?>
                        <div class="form-group col-md-3">
                            <label class="d-block" for="phantram">Phần trăm (S = <?=@$item['phantram']?>%):</label>
                            <div class="input-group">
                                <input type="text" class="form-control" name="data[phantram]" id="phantram" placeholder="Phần trăm" value="<?=@$item['phantram']?>">
                                <div class="input-group-append">
                                    <div class="input-group-text"><strong><?=@$item['phantram']?>%</strong></div>
                                </div>
                            </div>
                        </div>
                    <?php } ?>
                    <?php if(isset($config['news'][$type]['chondieukien']) && $config['news'][$type]['chondieukien'] == true) { ?>
                        <div class="form-group col-md-2">
                            <label class="d-block" for="id_loaicongtrinh">Chọn loại công trình:</label>
                            <?=$func->get_select_category('news','id_loaicongtrinh','loaicongtrinh',@$item['id_loaicongtrinh'])?>
                        </div>
                        <div class="form-group col-md-2">
                            <label class="d-block" for="id_dichvuxaynha">Chọn dịch vụ xây nhà:</label>
                            <?=$func->get_select_category('news','id_dichvuxaynha','dichvuxaynha',@$item['id_dichvuxaynha'])?>
                        </div>
                        <div class="form-group col-md-2">
                            <label class="d-block" for="id_mucdautu">Chọn mức đầu tư:</label>
                            <?=$func->get_select_category('news','id_mucdautu','mucdautu',@$item['id_mucdautu'])?>
                        </div>
                        <div class="form-group col-md-2">
                            <label class="d-block" for="id_mattien">Chọn mặt tiền:</label>
                            <?=$func->get_select_category('news','id_mattien','mattien',@$item['id_mattien'])?>
                        </div>
                    <?php } ?>
                    <?php if(isset($config['news'][$type]['gia']) && $config['news'][$type]['gia'] == true) { ?>
                        <div class="form-group col-md-2">
                            <label class="d-block" for="phantram">Đơn giá:</label>
                            <div class="input-group">
                                <input type="text" class="form-control format-price" name="data[gia]" id="gia" placeholder="Đơn giá" value="<?=@$item['gia']?>">
                                <div class="input-group-append">
