...
<section class="content">
    <div class="card-footer text-sm sticky-top">
        <?php if ((count($items) > 3 && $type=="news-project") || $type!="news-project") {?>
            ...
            <table class="table table-hover">
                <thead>
                    <tr>
                        ...
                        <?php } ?>
                        <th class="align-middle" style="width:<?=($type=='dongia')?'2':'3'?>0%">Tiêu đề</th>
                        <?php if(isset($config['news'][$type]['gallery']) && count($config['news'][$type]['gallery']) > 0) { ?>
                            <th class="align-middle">Gallery</th>
                        <?php } ?>
                        <?php if(isset($config['news'][$type]['theme']) && count($config['news'][$type]['theme']) > 0) { ?>
                            <th class="align-middle">Loại hiển thị</th>
                        <?php } ?>
                        <?php if(isset($config['news'][$type]['dientich']) && count($config['news'][$type]['dientich']) > 0) { ?>
                            <th class="align-middle">Diện tích</th>
                        <?php } ?>
                        <?php if(isset($config['news'][$type]['phantram_dongia']) && count($config['news'][$type]['phantram_dongia']) > 0) { ?>
                            <th class="align-middle">Phần trăm</th>
                        <?php } ?>
                        <?php if(isset($config['news'][$type]['chondieukien']) && count($config['news'][$type]['chondieukien']) > 0) { ?>
                            <th class="align-middle">Loại công trình</th>
                            <th class="align-middle">Dịch vụ xây nhà</th>
                            <th class="align-middle">Mức đầu tư</th>
                            <th class="align-middle">Mặt tiền</th>
                        <?php } ?>
                        <?php if(isset($config['news'][$type]['gia']) && count($config['news'][$type]['gia']) > 0) { ?>
                        ...
                        <?php for($i=0;$i<count($items);$i++) {
                            ...
                            <tr>
                                ...
                                <?php if(isset($config['news'][$type]['theme']) && count($config['news'][$type]['theme']) > 0) { ?>
                                    <td class="align-middle">
                                        <?php 
                                        $title_theme = 'Mặc định';
                                        if ($items[$i]['theme']==1) {
                                            $title_theme = 'Loại 2';
                                        }elseif($items[$i]['theme']==2){
                                            $title_theme = 'Loại slide';
                                        }
                                        echo $title_theme;
                                        ?>
                                    </td>
                                ...
                                <?php if(isset($config['news'][$type]['chondieukien']) && count($config['news'][$type]['chondieukien']) > 0) { ?>
                                    <td class="align-middle">
                                        <?=$func->get_select_category('news','id_loaicongtrinh','loaicongtrinh',@$items[$i]['id_loaicongtrinh'],true)?>
                                    </td>
                                    <td class="align-middle">
                                        <?=$func->get_select_category('news','id_dichvuxaynha','dichvuxaynha',@$items[$i]['id_dichvuxaynha'],true)?>
                                    </td>
                                    <td class="align-middle">
                                        <?=$func->get_select_category('news','id_mucdautu','mucdautu',@$items[$i]['id_mucdautu'],true)?>
                                    </td>
                                    <td class="align-middle">
                                        <?=$func->get_select_category('news','id_mattien','mattien',@$items[$i]['id_mattien'],true)?>
                                    </td>
                                <?php } ?>
                                <?php if(isset($config['news'][$type]['gia']) && count($config['news'][$type]['gia']) > 0) { ?>
                                    <td class="align-middle">
                                        <?=@$func->format_money($items[$i]['gia'])?>
                                    </td>
                                <?php } ?>
                                <?php if(isset($config['news'][$type]['check'])) { foreach($config['news'][$type]['check'] as $key => $value) { ?>
