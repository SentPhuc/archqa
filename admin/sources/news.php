...
function get_items()
{
	...
	foreach($config['news'][$type]['check'] as $column => $value)
	{
		$show_attr = $d->rawQueryOne("show columns from #_news like '".$column."' ");
		if($show_attr == null) $d->rawQuery("alter table #_news ADD ".$column." int(2) DEFAULT 0 AFTER noibat");
	}
...
function save_item()
{
	...
	if($id && $act!='save_copy')
	{
		...
		if(isset($_FILES['file1']))
		{
			...
			if($photo1 = $func->uploadImage("file1", $config['news'][$type]['img_type'], UPLOAD_NEWS,$file1_name))
			{
				$data['photo1'] = $photo1;
				$row = $d->rawQueryOne("select id, photo1 from #_news where id = ? and type = ? limit 0,1",array($id,$type));
				if(isset($row['id']) && $row['id'] > 0) $func->delete_file(UPLOAD_NEWS.$row['photo1']);
			}
		}
	...
	else
	{
		...
		{
			$file1_name = $func->uploadName($_FILES['file1']["name"]);
			if($photo1 = $func->uploadImage("file", $config['news'][$type]['img_type'], UPLOAD_NEWS,$file1_name))
			{
				$data['photo1'] = $photo1;
			}
		}
...
function delete_item()
{
	...
	if($id)
	{
		...
		/* Lấy dữ liệu */
		$row = $d->rawQueryOne("select id, photo,photo1 from #_news where id = ? and type = ? limit 0,1",array($id,$type));

		if(isset($row['id']) && $row['id'] > 0)
		{
			$func->delete_file(UPLOAD_NEWS.$row['photo']);
			$func->delete_file(UPLOAD_NEWS.$row['photo1']);
...
function get_lists()
{
	...
	foreach($config['news'][$type]['check_list'] as $column => $value)
	{
		$show_attr = $d->rawQueryOne("show columns from #_news_list like '".$column."' ");
		if($show_attr == null) $d->rawQuery("alter table #_news_list ADD ".$column." int(2) DEFAULT 0 AFTER noibat");
	}
...
function save_list()
{
	...
	if($id)
	{
		...
		{
			$file1_name = $func->uploadName($_FILES['file1']["name"]);
			if($photo1 = $func->uploadImage("file1", $config['news'][$type]['img_type_list'], UPLOAD_NEWS,$file1_name))
			{
				$data['photo1'] = $photo1;
				$row = $d->rawQueryOne("select id, photo1 from #_news_list where id = ? and type = ? limit 0,1",array($id,$type));
				if(isset($row['id']) && $row['id'] > 0) $func->delete_file(UPLOAD_NEWS.$row['photo1']);
			}
		}
		...
		if(isset($_FILES['file2']))
		{
			...
			if($photo2= $func->uploadImage("file2", $config['news'][$type]['img_type_list'], UPLOAD_NEWS,$file2_name))
			{
				$data['photo2'] = $photo2;
				$row = $d->rawQueryOne("select id, photo1 fr2m #_news_list where id = ? and type = ? limit 0,1",array($id,$type));
...
function delete_list()
{
	...
	if($id)
	{
		...
		/* Lấy dữ liệu */
		$row = $d->rawQueryOne("select id, photo,photo1,photo2 from #_news_list where id = ? and type = ? limit 0,1",array($id,$type));

		if(isset($row['id']) && $row['id'] > 0)
		{
			$func->delete_file(UPLOAD_NEWS.$row['photo']);
			$func->delete_file(UPLOAD_NEWS.$row['photo1']);
			$func->delete_file(UPLOAD_NEWS.$row['photo2']);
...
function get_cats()
{
	...
	foreach($config['news'][$type]['check_cat'] as $column => $value)
	{
		$show_attr = $d->rawQueryOne("show columns from #_news_cat like '".$column."' ");
		if($show_attr == null) $d->rawQuery("alter table #_news_cat ADD ".$column." int(2) DEFAULT 0 AFTER noibat");
	}
...
function get_loais()
{
	...
	foreach($config['news'][$type]['check_item'] as $column => $value)
	{
		$show_attr = $d->rawQueryOne("show columns from #_news_item like '".$column."' ");
		if($show_attr == null) $d->rawQuery("alter table #_news_item ADD ".$column." int(2) DEFAULT 0 AFTER noibat");
	}
...
function get_subs()
{
	...
	foreach($config['news'][$type]['check_sub'] as $column => $value)
	{
		$show_attr = $d->rawQueryOne("show columns from #_news_sub like '".$column."' ");
		if($show_attr == null) $d->rawQuery("alter table #_news_sub ADD ".$column." int(2) DEFAULT 0 AFTER noibat");
	}
