		...
		else
		{
			...
			if($numberPhoto && $dataMultiTemp)
			{
				...
				for($i=0;$i<count($dataMultiTemp);$i++)
				{
					...
					if ($type=="product" && $val=="tuy-chinh" && !empty($dataMultiTemp[$i]['tenvi'])) {
						$success_nomal = 1;
					}

					if ($type=="news-project" && $val=="thongke" && !empty($dataMultiTemp[$i]['value'])) {
						$success_nomal = 1;
					}

					if($success_file || $success_photo || $success_video || $success_nomal)
					{
