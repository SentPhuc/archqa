...
{
	global $d, $func, $config, $type,$myPermissions;
	...
	/* Xóa cache watermark */
	if(strlen(strstr($type, "watermark")) > 0 || $myPermissions==true) {
...
function get_photos()
{
	...
	if (!empty($config['photo']['man_photo'][$type]['check_photo'])) {
		foreach($config['photo']['man_photo'][$type]['check_photo'] as $column => $value)
		{
			$show_attr = $d->rawQueryOne("show columns from table_photo like '".$column."' ");
			if($show_attr == null) $d->rawQuery("alter table table_photo ADD ".$column." int(2) DEFAULT 0 AFTER noibat");
		}
	}
	...
	else
	{
		...
		if($numberPhoto && $dataMultiTemp)
		{
			...
			for($i=0;$i<count($dataMultiTemp);$i++)
			{
				...
				if ($type=="news-quality" && $dataMultiTemp[$i]['tenvi']) {
					$success_photo = 1;
				}
