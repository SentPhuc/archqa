...
function get_items()
{
	...
	foreach($config['product'][$type]['check'] as $column => $value)
	{
		$show_attr = $d->rawQueryOne("show columns from table_product like '".$column."' ");
		if($show_attr == null) $d->rawQuery("alter table table_product ADD ".$column." int(2) DEFAULT 0 AFTER noibat");
	}
...
function save_item()
{
	...
	$filter_group = (isset($_POST['filter_group'])) ? $_POST['filter_group'] : null;
	if($data)
	{
...
function get_lists()
{
	...
	$where = "";
	foreach($config['product'][$type]['check_list'] as $column => $value)
	{
		$show_attr = $d->rawQueryOne("show columns from #_product_list like '".$column."' ");
		if($show_attr == null) $d->rawQuery("alter table #_product_list ADD ".$column." int(2) DEFAULT 0 AFTER noibat");
	}
	if(isset($_REQUEST['keyword']))
	{
		$keyword = htmlspecialchars($_REQUEST['keyword']);
...
function save_list()
{
	...
	if($data)
	{
		...
		if(isset($config['product'][$type]['filter_list']) && $config['product'][$type]['filter_list'] == true)
		{
			if(isset($_POST['filter_group']) && ($_POST['filter_group'] != '')) $data['id_filter'] = implode(",", $_POST['filter_group']);
			else $data['id_filter'] = "";
		}
	...
	if($id)
	{
		...
		if(isset($_FILES['file1']))
		{
			...
			if($photo1 = $func->uploadImage("file1", $config['product'][$type]['img_type'], UPLOAD_PRODUCT,$file1_name))
			{
				$data['photo1'] = $photo1;
				$row = $d->rawQueryOne("select id, photo1 from #_product where id = ? and type = ? limit 0,1",array($id,$type));
				if(isset($row['id']) && $row['id'] > 0) $func->delete_file(UPLOAD_PRODUCT.$row['photo1']);
			}
		}

		if(isset($_FILES['file2']))
		{
			$file2_name = $func->uploadName($_FILES['file2']["name"]);
			if($photo2 = $func->uploadImage("file2", $config['product'][$type]['img_type'], UPLOAD_PRODUCT,$file2_name))
			{
				$data['photo2'] = $photo2;
		...
		{
			$file3_name = $func->uploadName($_FILES['file3']["name"]);
			if($photo3 = $func->uploadImage("file3", $config['product'][$type]['img_type'], UPLOAD_PRODUCT,$file3_name))
			{
				$data['photo3'] = $photo3;
		...
		{
			$file4_name = $func->uploadName($_FILES['file4']["name"]);
			if($photo4 = $func->uploadImage("file4", $config['product'][$type]['img_type'], UPLOAD_PRODUCT,$file4_name))
			{
				$data['photo4'] = $photo4;
				$row = $d->rawQueryOne("select id, photo4 from #_product where id = ? and type = ? limit 0,1",array($id,$type));
				if(isset($row['id']) && $row['id'] > 0) $func->delete_file(UPLOAD_PRODUCT.$row['photo4']);
			}
		}
...
function delete_list()
{
	...
	if($id)
	{
		...
		/* Lấy dữ liệu */
		$row = $d->rawQueryOne("select id, photo,photo1,photo2,photo3,photo4 from #_product_list where id = ? and type = ? limit 0,1",array($id,$type));

		if(isset($row['id']) && $row['id'] > 0)
		{
			$func->delete_file(UPLOAD_PRODUCT.$row['photo']);
			$func->delete_file(UPLOAD_PRODUCT.$row['photo1']);
			$func->delete_file(UPLOAD_PRODUCT.$row['photo2']);
			$func->delete_file(UPLOAD_PRODUCT.$row['photo3']);
			$func->delete_file(UPLOAD_PRODUCT.$row['photo4']);
...
function get_cats()
{
	...
	foreach($config['product'][$type]['check_cat'] as $column => $value)
	{
		$show_attr = $d->rawQueryOne("show columns from #_product_cat like '".$column."' ");
		if($show_attr == null) $d->rawQuery("alter table #_product_cat ADD ".$column." int(2) DEFAULT 0 AFTER noibat");
	}
...
function get_loais()
{
	...
	foreach($config['product'][$type]['check_item'] as $column => $value)
	{
		$show_attr = $d->rawQueryOne("show columns from #_product_item like '".$column."' ");
		if($show_attr == null) $d->rawQuery("alter table #_product_item ADD ".$column." int(2) DEFAULT 0 AFTER noibat");
	}
...
function get_subs()
{
	...
	foreach($config['product'][$type]['check_sub'] as $column => $value)
	{
		$show_attr = $d->rawQueryOne("show columns from #_product_sub like '".$column."' ");
		if($show_attr == null) $d->rawQuery("alter table #_product_sub ADD ".$column." int(2) DEFAULT 0 AFTER noibat");
	}
